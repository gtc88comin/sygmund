                ...
                <div key={c.id} style={styles.card}>
                  ...
                  <div style={styles.infoBox}>
                    ...
                    <div style={{ marginTop: '15px', display: 'flex', gap: '10px' }}>
                      ...
                      <button
                        onClick={() => {
                          const horaInicio = c.hora.replace(':', '');
                          const dataLimpa = c.data.replace(/-/g, '');
                          const inicio = `${dataLimpa}T${horaInicio}00`;
                          const fim = `${dataLimpa}T${Number(horaInicio) + 100}00`; // +1 hora estimado
                          const titulo = encodeURIComponent(`Consulta: ${c.pacienteNome}`);
                          const detalhes = encodeURIComponent(`Consulta Sygmund com ${c.pacienteNome}.\nObs: ${c.obs || ''}`);
                          window.open(`https://calendar.google.com/calendar/render?action=TEMPLATE&text=${titulo}&dates=${inicio}/${fim}&details=${detalhes}`, '_blank');
                        }}
                        style={styles.actionBtnSecundary}
                      >
                        📅 Google
                      </button>
                      ...
                      {c.pacienteTelefone && (
                        <button
                          onClick={() => {
                            const numbers = c.pacienteTelefone.replace(/\D/g, '');
                            const fullNumber = numbers.length <= 11 ? `55${numbers}` : numbers;
                            const msg = encodeURIComponent(`Olá ${c.pacienteNome}, confirmando sua consulta para dia ${new Date(c.data + 'T00:00:00').toLocaleDateString('pt-BR')} às ${c.hora}. Responda este contato para confirmar.`);
...
const styles = {
  ...
  actionBtnPrimary: { padding: "8px 12px", backgroundColor: "#25D366", color: "#000", border: "none", borderRadius: "4px", fontSize: "12px", fontWeight: "bold", cursor: "pointer" },
  actionBtnSecundary: { padding: "8px 12px", backgroundColor: "#fff", color: "#333", border: "none", borderRadius: "4px", fontSize: "12px", fontWeight: "bold", cursor: "pointer" },
